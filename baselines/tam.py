...
## TAM ##
@torch.no_grad()
def compute_tam(output, edge_index, label, train_mask, aggregator, cls_num_list=None, temp_phi = None, temp_gamma = None):
    n_cls = label.max().item() + 1

    # Apply class-wise temperature
    # cls_num_list = torch.FloatTensor(class_num_list).to(output.device)
    cls_num_list = cls_num_list.to(output.device)
...
def adjust_output(args, output, edge_index, label, train_mask, aggregator, class_num_list, epoch):
    ...
    if args.tam and epoch > args.warmup:
        acm, adm = compute_tam(output, edge_index, label, train_mask, aggregator, \
                                cls_num_list=class_num_list, temp_phi = args.temp_phi, temp_gamma = 0.4)

    # Adjust outputs
    if args.tam and epoch > args.warmup:
        acm = acm.log()
        adm = - adm
        output[train_mask] = output[train_mask] + args.tam_alpha*acm + args.tam_beta*adm
